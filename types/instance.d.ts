...
export interface FastifyListenOptions {
   ...
   * settings such as `tcp_max_syn_backlog` and `somaxconn` on Linux.
   * Default to `511`.
   */
  backlog?: number;
  /**
  ...
  writableAll?: boolean;
  /**
   * For TCP servers, setting `ipv6Only` to `true` will disable dual-stack support,
   * i.e., binding to host `::` won't make `0.0.0.0` be bound.
   * Default to `false`.
   */
  ipv6Only?: boolean;
  /**
...
type DecorationMethod<This, Return = This> = {
  <
    // Need to disable "no-use-before-define" to maintain backwards compatibility,
    // as else decorate<Foo> would suddenly mean something new
...
> {
  ...
  // should be able to define something useful with the decorator getter/setter pattern
  // using Generics to enforce the users function returns what they expect it to
  ...
  /**
   * `onRequest` is the first hook to be executed in the request lifecycle.
   * There was no previous hook, the next hook will be `preParsing`.
  ...
  /**
   * `preParsing` is the second hook to be executed in the request lifecycle.
   * The previous hook was `onRequest`, the next hook will be `preValidation`.
  ...
  /**
   * `preValidation` is the third hook to be executed in the request lifecycle.
   *  The previous hook was `preParsing`, the next hook will be `preHandler`.
   */
  addHook<
    RouteGeneric extends RouteGenericInterface = RouteGenericInterface,
    ContextConfig = ContextConfigDefault,
  ...
  /**
   * `preHandler` is the fourth hook to be executed in the request lifecycle.
   * The previous hook was `preValidation`, the next hook will be `preSerialization`.
   */
  addHook<
    RouteGeneric extends RouteGenericInterface = RouteGenericInterface,
    ContextConfig = ContextConfigDefault,
   ...
   * It is the sixth hook to be executed in the request lifecycle.
   * The previous hook was `preSerialization`, the next hook will be `onResponse`.
  ...
  /**
   * `onResponse` is the seventh and last hook in the request hook lifecycle.
   * The previous hook was `onSend`, there is no next hook.
   * The onResponse hook is executed when a response has been sent, so you will
   * not be able to send more data to the client. It can however be useful for
   * sending data to external services, for example to gather statistics.
   */
  addHook<
    RouteGeneric extends RouteGenericInterface = RouteGenericInterface,
    ContextConfig = ContextConfigDefault,
  ...
  /**
   * `onRequestAbort` is useful if you need to monitor the if the client aborts
   * the request (if the `request.raw.aborted` property is set to `true`).
   * The `onRequestAbort` hook is executed when a client closes the connection before
   * the entire request has been received. Therefore, you will not be able to send data to the client.
   * Notice: client abort detection is not completely reliable.
   * See: https://github.com/fastify/fastify/blob/main/docs/Guides/Detecting-When-Clients-Abort.md
   */
  addHook<
    RouteGeneric extends RouteGenericInterface = RouteGenericInterface,
    ContextConfig = ContextConfigDefault,
   ...
   * This hook will be executed only after the customErrorHandler has been executed, and only if
   * the customErrorHandler sends an error back to the user (Note that the default customErrorHandler always
  ...
  /**
   * Triggered when a new route is registered. Listeners are passed a routeOptions object as the sole parameter.
   * The interface is synchronous, and, as such, the listener does not get passed a callback
   */
  addHook<
    RouteGeneric extends RouteGenericInterface = RouteGenericInterface,
    ContextConfig = ContextConfigDefault,
   ...
   * This hook can be useful if you are developing a plugin that needs to know when a plugin context
   * is formed, and you want to operate in that specific context.
  ...
  /**
   * Triggered when fastify.listen() or fastify.ready() is invoked to start the server.
   * It is useful when plugins need a "ready" event, for example to load data before
  ...
  /**
   * Triggered when fastify.listen() is invoked to start the server.
   * It is useful when plugins need a "onListen" event, for example to run logics
   * after the server start listening for requests.
   */
  addHook<
    Fn extends
  ...
  /**
   * Triggered when fastify.close() is invoked to stop the server.
   * It is useful when plugins need to cancel some state to allow the server to close successfully.
   */
