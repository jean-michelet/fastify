      ...
      for (const adr of addresses) {
        if (adr.address !== primaryAddress.address) {
          ...
          const closeSecondary = () => {
            ...
            if (
              typeof secondaryServer.closeAllConnections === 'function' &&
              serverOpts.forceCloseConnections === true
            ) {
  ...
  return this.ready().then(() => {
    ...
    const listeningEvent = new Promise((resolve, reject) => {
      listeningEventHandler = () => {
        ...
        this[kState].listening = true
        resolve(
          logServerAddress.call(this, server, listenOptions.listenTextResolver || defaultResolveServerListeningText)
        )
      }
