...
const { FST_ERR_REP_INVALID_PAYLOAD_TYPE, FST_ERR_FAILED_ERROR_SERIALIZATION } = require('./errors')
    ...
    fallbackErrorHandler(error, reply, function (reply, payload) {
      try {
        ...
        if (!reply.log[kDisableRequestLogging]) {
          reply.log.warn({ req: reply.request, res: reply, err: error }, error && error.message)
        }
...
function fallbackErrorHandler (error, reply, cb) {
  ...
  try {
    ...
    if (!reply.log[kDisableRequestLogging]) {
      // error is always FST_ERR_SCH_SERIALIZATION_BUILD because this is called
      // from route/compileSchemasForSerialization
