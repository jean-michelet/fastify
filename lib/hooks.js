...
/**
 * Adds a lifecycle or application hook to the Fastify instance.
 *
 * @param {string} name - The name of the hook (e.g. `'onRequest'`, `'onSend'`).
 * @param {function} fn - The hook function.
 * @returns {this}
 * @throws {FST_ERR_HOOK_INVALID_HANDLER}
 * @throws {FST_ERR_HOOK_INVALID_ASYNC_HANDLER}
 */
function addHook (name, fn) {
  if (fn == null) {
    throw new FST_ERR_HOOK_INVALID_HANDLER(name, fn)
  }
  ...
  if (name === 'onClose') {
    ...
    this.after((err, done) => {
      addHookRecursively.call(this, name, fn)
      done(err)
    })
  }
  return this

  function addHookRecursively (name, fn) {
    this[kHooks].add(name, fn)
    this[kChildren].forEach(child => addHookRecursively.call(child, name, fn))
  }
}
