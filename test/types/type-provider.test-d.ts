...
interface AuxiliaryPluginProvider extends FastifyTypeProvider {
  validator: 'plugin-auxiliary';
}

// Auxiliary plugins may have varying server types per application.
// Recommendation would be to explicitly remap instance provider context within plugin if required.
...
interface AuxiliaryHandlerProvider extends FastifyTypeProvider {
  validator: 'handler-auxiliary';
}

// Auxiliary handlers are likely shared for multiple routes and thus should infer
// as unknown due to potential varying parameters
