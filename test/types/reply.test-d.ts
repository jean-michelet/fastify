...
const getHandler: RouteHandlerMethod = function (_request, reply) {
  ...
  expectType<(hints: Record<string, string | string[]>, callback?: (() => void) | undefined) => void>(
  ...
  expectType<
    ...
    key: string,
    fn:
        | ((reply: FastifyReply, payload: string | Buffer | null) => Promise<string>)
        | ((
          reply: FastifyReply,
  ...
  expectAssignable<(schema: { [key: string]: unknown }) => DefaultSerializationFunction | undefined>(
    reply.getSerializationFunction
  )
  expectAssignable<(schema: { [key: string]: unknown }, httpStatus?: string) => DefaultSerializationFunction>(
