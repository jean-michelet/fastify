...
test('reply.serializer should support running preSerialization hooks', (t, done) => {
  ...
  fastify.addHook('preSerialization', async (request, reply, payload) => {
    t.assert.ok('called', 'preSerialization')
  })
  fastify.route({
    method: 'GET',
    url: '/',
    handler: (req, reply) => {
      reply.type('application/json').serializer(JSON.stringify).send({ foo: 'bar' })
    }
  })
...
test('within an instance', async (t) => {
  ...
  await t.test('redirect to `/` - 9', (t, done) => {
    ...
    http.get(
      'http://127.0.0.1:' + fastify.server.address().port + '/redirect-code-before-call-overwrite',
      function (response) {
        t.assert.strictEqual(response.statusCode, 302)
        done()
      }
...
test('plain string with custom json content type should NOT be serialized as json', async (t) => {
  ...
  await Promise.all(
    Object.keys(customSamples).map((path) => {
      return new Promise((resolve, reject) => {
        sget(
          {
            method: 'GET',
...
test('Reply should handle JSON content type with a charset', async (t) => {
  ...
  fastify.get('/utf8', function (req, reply) {
    reply.header('content-type', 'application/json; charset=utf-8').send({ hello: 'world' })
  })

  fastify.get('/utf16', function (req, reply) {
    reply.header('content-type', 'application/json; charset=utf-16').send({ hello: 'world' })
  })

  fastify.get('/utf32', function (req, reply) {
    reply.header('content-type', 'application/json; charset=utf-32').send({ hello: 'world' })
  })
