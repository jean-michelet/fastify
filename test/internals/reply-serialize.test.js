...
test('Reply#compileSerializationSchema', async (t) => {
  ...
  await t.test('Should reuse the serialize fn across multiple invocations - Route without schema', async (t) => {
    const fastify = Fastify()
    let serialize = null
    let counter = 0
  ...
  await t.test('Should use the custom serializer compiler for the route', async (t) => {
    const fastify = Fastify()
    let called = 0
...
test('Reply#getSerializationFunction', async (t) => {
  ...
  await t.test('Should retrieve the serialization function from the Schema definition', async (t) => {
    const fastify = Fastify()
    const okInput201 = {
      status: 'ok',
      message: 'done!'
      ...
      (req, reply) => {
        ...
        if (Number(id) === 1) {
          ...
          try {
            ...
            t.assert.strictEqual(err.message, 'The value "something" cannot be converted to an integer.')
          }
  ...
  await t.test('Should retrieve the serialization function from the cached one', async (t) => {
    const fastify = Fastify()
...
test('Reply#serializeInput', async (t) => {
  ...
  await t.test('Should throw if missed serialization function from HTTP status', async (t) => {
    const fastify = Fastify()
  ...
  await t.test('Should use a serializer fn from HTTP status', async (t) => {
    ...
    fastify.get(
      ...
      (req, reply) => {
        t.assert.strictEqual(reply.serializeInput(okInput4xx, '4xx'), JSON.stringify(okInput4xx))
        t.assert.strictEqual(reply.serializeInput(okInput201, 201), JSON.stringify(okInput201))
  ...
  await t.test('Should compile a serializer out of a schema if serializer fn missed', async (t) => {
    ...
    fastify.get('/', { serializerCompiler }, (req, reply) => {
      t.assert.strictEqual(reply.serializeInput(testInput, schemaObj), JSON.stringify(testInput))
  ...
  await t.test('Should use a cached serializer fn', async (t) => {
    ...
    fastify.get('/', { serializerCompiler }, (req, reply) => {
      ...
      t.assert.strictEqual(reply.serializeInput(testInput, schemaObj), cached(testInput))
